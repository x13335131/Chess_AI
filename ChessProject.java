...
private Stack getWhitePawnSquares(int x, int y, String piece){
  ...
  Move validM, validM2, validM3, validM4; //make four possible moves

  // For pawn is at starting position
    for(int i = 1; i <=2; i++) { //i is less than/equal to 2, increment i
        int tmpy = y + i; //tempy is equal to starting y + the i number of spaces
        int tmpx = x;

        if(!(tmpy > 7)) { //if piece is not off the board
            // making pawn move twice from starting position
            if(y == 1) {//if y coordinate is equal to 1
                Square tmp = new Square(tmpx, tmpy, piece);//new square
                validM = new Move(startingSquare, tmp); //validM is equal to a new move taking in starting square and the new assigned square as params
                if (!piecePresent(((tmp.getXC() * 75) + 20), (((tmp.getYC() * 75) + 20)))) {//if no piece present or in the way
                    moves.push(validM);//make the first move
                }
            }
        }
      }
      //For pawn is not at starting position
        for (int i = 1; i < 2; i++){// i is LESS than 2, (only move one space now)
            int tmpy = y + i;
            int tmpx = x;

            if (!(tmpy > 7)) {//if not off the board
                Square tmp = new Square(tmpx, tmpy, piece);
                validM2 = new Move(startingSquare, tmp); //validM is equal to a new move taking in starting square and the new assigned square as params
                if (!piecePresent(((tmp.getXC() * 75) + 20), (((tmp.getYC() * 75) + 20)))) { //if no piece present or in the way
                    moves.push(validM2);//make the move
                }
            }
        }
     //For pawn is not at starting position  & wants to make a diagonal move bottom right
        for(int i = 1; i < 2; i++) {
            int tmpy = y + i; //up board
            int tmpx = x + i; //across 1 square right
            //if piece is not off the board
            if (!(tmpx > 7 || tmpx < 0 || tmpy > 7 || tmpy < 0)) {
                Square tmp = new Square(tmpx, tmpy, piece);
                validM3 = new Move(startingSquare, tmp);
                if (piecePresent(((tmp.getXC() * 75) + 20), (((tmp.getYC() * 75) + 20)))) {//if a piece is present
                    if (checkWhiteOponent(((tmp.getXC() * 75) + 20), (((tmp.getYC() * 75) + 20)))) { //if piece is opponent piece
                        moves.push(validM3);//make move
                    }
                }
            }
        }
        //For pawn is not at starting position & wants to make a diagonal move bottom left
        for(int i = 1; i < 2; i++) {
            int tmpy = y + i; //up one
            int tmpx = x - i; //across 1 square left
            if(!(tmpx > 7 || tmpx < 0 || tmpy > 7 || tmpy < 0)) {
                Square tmp = new Square(tmpx, tmpy, piece);
                validM4 = new Move(startingSquare, tmp);
                if (piecePresent(((tmp.getXC() * 75) + 20), (((tmp.getYC() * 75) + 20)))) { //checking if a piece is present
                    if (checkWhiteOponent(((tmp.getXC() * 75) + 20), (((tmp.getYC() * 75) + 20)))) { //checking if it is an enemy piece
                        moves.push(validM4);
                    }
                }
            }
...
private void printStack(Stack input){
  ...
  private void makeAIMove(){
    ...
    */
    resetBorders();//reset the borders- change the colors of borders back to blank
    layeredPane.validate(); //validated
    layeredPane.repaint(); //repainted
    Stack white = findWhitePieces(); //find all white pieces. stack of where all white pieces start from
    Stack black = findBlackPieces();
    Stack completeMoves = new Stack(); //instanciate a new empty stack for complete movements
    Move tmp;
    while(!white.empty()){ //while we have potential pieces still in the stack, we will loop through that
      Square s = (Square)white.pop(); //pop them off to a potential square
      String tmpString = s.getName();//get name
      Stack tmpMoves = new Stack(); //get stack associated with the square
      Stack temporary = new Stack();
      /*
          We need to identify all the possible moves that can be made by the AI Opponent
      */
      //see if pieces sitting on a square is any of the following
